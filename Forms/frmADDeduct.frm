   ...
   Begin VB.Frame fra1 
      ...
      Begin TDBText6Ctl.TDBText txtSearch 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         DropDown        =   "frmADDeduct.frx":68BE
         Key             =   "frmADDeduct.frx":68DC
      ...
      Begin TrueOleDBList80.TDBCombo tdbPayrollPeriod 
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmADDeduct.frx":6920
      ...
      Begin TrueOleDBList80.TDBCombo tdbSort 
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmADDeduct.frx":69CA
         _StyleDefs(0)   =   "_StyleRoot:id=0,.parent=-1,.alignment=3,.valignment=0,.bgcolor=&H80000005&"
         _StyleDefs(1)   =   ":id=0,.fgcolor=&H80000008&,.wraptext=0,.locked=0,.transparentBmp=0"
         _StyleDefs(2)   =   ":id=0,.fgpicPosition=0,.bgpicMode=0,.appearance=0,.borderSize=0,.ellipsis=0"
         _StyleDefs(3)   =   ":id=0,.borderColor=&H80000005&,.borderType=0,.bold=0,.fontsize=825,.italic=0"
      ...
      End
   End
   Begin TrueOleDBGrid80.TDBGrid tdgDeductions 
      Height          =   2550
      Left            =   15
      TabIndex        =   7
      Top             =   2295
      ...
      Columns(2)._VlistStyle=   0
      Columns(2)._MaxComboItems=   5
      Columns(2).Caption=   "Other Deductions"
      Columns(2).DataField=   "otherdeductions"
      Columns(2)._PropDict=   "_MaxComboItems,516,2;_VlistStyle,514,3"
      Columns(3)._VlistStyle=   0
      Columns(3)._MaxComboItems=   5
...
Public rsDeductions   As ADODB.Recordset
  ...
  Select Case Index
    ...
    Case 0:
          ...
          If Trim(tdbPayrollPeriod.Text) <> "" And Not IsNull(tdbPayrollPeriod.SelectedItem) And tdbPayrollPeriod.ApproxCount > 0 Then
            frmADDeduct2.mAdd = True
            frmADDeduct2.Show vbModal
          Else
...
Private Sub Form_Load()
    ...
    bind_tdb ConMain, tdbPayrollPeriod, "select percode, description, wrkdatefrom, wrkdateto,payyear,paymonth from payrollperiod order by description", "description", "percode"
    ...
    With rsTmp
      For I = 1 To 6
        ...
        Select Case I
          ...
          Case 3: .Fields("code") = "otherdeductions"
                  .Fields("description") = "Other deductions"
...
Private Sub Cancel_Clicked()
  If rsDeductions.RecordCount > 0 Then
...
Private Sub tdbPayrollPeriod_LostFocus()
  ...
  If Not IsNull(tdbPayrollPeriod.SelectedItem) And Trim(tdbPayrollPeriod.Text) <> "" And tdbPayrollPeriod.ApproxCount > 0 Then
    ...
    NetOpen rsDeductions, "select x1.*,x2.costcenter,x3.division,x4.branch,x6.otherdeductions, " & _
                          "concat(x5.lastname,', ',x5.firstname,' ',x5.middlename) fullname from deductions x1  " & _
                          ...
                          "left outer join otherdeductions x6 on x1.otherdeductionscode = x6.otherdeductionscode " & _
                          "where x1.percode = '" & tdbPayrollPeriod.BoundText & "' order by x1.percode"
  
    Set tdgDeductions.DataSource = rsDeductions
...
Private Sub tdbPayrollPeriod_ItemChange()
  ...
  Set tdgDeductions.DataSource = Nothing
...
Private Sub tdbSort_ItemChange()
  rsDeductions.Sort = tdbSort.BoundText
End Sub
...
Private Sub txtSearch_KeyPress(KeyAscii As Integer)
  If KeyAscii = 13 Then
    SendKeys "{TAB}"
  Else
    SearchRecord KeyAscii, txtSearch, rsDeductions, tdbSort.BoundText, tdbSort.BoundText
  End If
End Sub
