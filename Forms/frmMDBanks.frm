...
Begin VB.Form frmMDBanks 
   ...
   Begin VB.PictureBox pic1 
      ...
      End
   End
   Begin TrueOleDBGrid80.TDBGrid tdgBank 
      Height          =   3750
      Left            =   15
      TabIndex        =   8
      Top             =   2625
      ...
      Columns(2).DataField=   "bankdescription"
   ...
   Begin VB.Frame fraInfo 
      ...
      Top             =   630
      Width           =   12765
      Begin TDBText6Ctl.TDBText txtBankDummyCode 
         Height          =   300
         Left            =   2265
         TabIndex        =   10
         Top             =   225
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         DropDown        =   "frmMDBanks.frx":6CAE
         Key             =   "frmMDBanks.frx":6CCC
         BackColor       =   14737632
         EditMode        =   0
         ForeColor       =   4210752
         ReadOnly        =   1
      ...
      Begin TDBText6Ctl.TDBText txtBankName 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         DropDown        =   "frmMDBanks.frx":6D7C
         Key             =   "frmMDBanks.frx":6D9A
         ...
         OLEDropMode     =   0
      End
      Begin TDBText6Ctl.TDBText txtBankDescription 
         Height          =   300
         Left            =   2265
         TabIndex        =   12
         Top             =   915
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         DropDown        =   "frmMDBanks.frx":6E4A
         Key             =   "frmMDBanks.frx":6E68
         BackColor       =   -2147483643
         EditMode        =   0
         ForeColor       =   4210752
         ReadOnly        =   0
   ...
   Begin VB.Frame fraSearch 
      ...
      Begin TDBText6Ctl.TDBText txtSearch 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         DropDown        =   "frmMDBanks.frx":6F18
         Key             =   "frmMDBanks.frx":6F36
         BackColor       =   -2147483643
         EditMode        =   0
...
Private Sub Form_Load()
    ...
    NetOpen rsBank, "select * from Bank order by dummycode"
    If rsBank.RecordCount > 0 Then
      rsBank.MoveFirst
    End If
    Set tdgBank.DataSource = rsBank

    CreateTmpDB rsTmp
    
    With tdgBank
        For i = .Columns("dummycode").ColIndex To .Columns("Bankdescription").ColIndex
...
Private Sub AddSave_Button_Clicked()
    ...
    If cmdMenu(0).Caption = "&NEW" Then
        ...
        tdgBank.Enabled = False
        fraSearch.Enabled = False
        txtBankDummyCode.Text = "Auto Generated..."
    ...
    Else
        ...
        NetOpen rsCheck, "select * from Bank where Bankname = '" & Swap(txtBankName.Text) & "'"
        
        If rsCheck.RecordCount > 0 Then
            If MsgBox("Bank name already exist. Would you like to continue saving this entry?", vbQuestion + vbYesNo) = vbNo Then
                Exit Sub
            End If
        End If
        ...
        ConMain.BeginTrans
        mCode = LastCode("Bank")
        txtBankDummyCode.Text = mCode
        ConMain.Execute "insert into Bank (Bankcode,dummycode,Bankname,Bankdescription) values  " & _
                          "(" & txtBankDummyCode.Text & ",'" & Format(txtBankDummyCode.Text, "0000000") & "','" & Swap(txtBankName.Text) & "','" & Swap(txtBankDescription.Text) & "')"
        
        txtBankDummyCode.Text = Format(txtBankDummyCode.Text, "0000000")
      
        ConMain.CommitTrans
        rsBank.Requery
        rsBank.Find "Bankcode = " & mCode & ""
...
Private Sub EditUpdate_Button_Clicked()
    ...
    Else
        ...
        mCode = rsBank!bankcode
        
        NetOpen rsCheck, "select * from Bank where Bankcode <> " & mCode & " and Bankname = '" & Swap(txtBankName.Text) & "'"
        ...
        ConMain.Execute "update Bank set Bankname = '" & Swap(txtBankName.Text) & "',Bankdescription = '" & Swap(txtBankDescription.Text) & "'  where Bankcode = " & mCode & ""
        ConMain.Execute "insert into eventslog (modulename,actiontaken,trnxdate,usercode,username) values " & _
                          "('" & Me.Name & "','" & cmdMenu(1).Caption & " - " & txtBankDummyCode.Text & "', Now()," & GlobalUserID & ",'" & UserName & "')"
...
Private Sub Cancel_Clicked()
  ...
  cmdMenu(1).Caption = "&EDIT"
    ...
    tdgBank.Enabled = True
    fraSearch.Enabled = True
    tdgBank_RowColChange 0, 0
...
Private Sub tdbSearch_ItemChange()
    rsBank.Sort = tdbSearch.BoundText
End Sub
...
Private Sub tdgBank_HeadClick(ByVal ColIndex As Integer)
    If rsBank.RecordCount > 0 Then
      tdbSearch.BoundText = tdgBank.Columns(ColIndex).DataField
      rsBank.Sort = tdbSearch.BoundText
    End If
End Sub

Private Sub tdgBank_RowColChange(LastRow As Variant, ByVal LastCol As Integer)

  With rsBank
    If .RecordCount > 0 Then
      txtBankDummyCode.Text = !dummycode
      txtBankName.Text = !Bankname
      txtBankDescription.Text = !BankDescription
    Else
      ClearText
    End If
  End With
...
Private Sub txtBankDescription_Keypress(KeyAscii As Integer)
    If KeyAscii = 13 Then
...
Private Sub txtBankDummyCode_Keypress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        SendKeys "{TAB}"
    End If
End Sub
...
Private Sub txtSearch_KeyPress(KeyAscii As Integer)
  If KeyAscii = 13 Then
    SendKeys "{TAB}"
  Else
    SearchRecord KeyAscii, txtSearch, rsBank, txtSearch.Text, tdbSearch.BoundText
  End If
End Sub

Private Sub ClearText()

  txtBankDummyCode.Text = ""
  txtBankName.Text = ""
...
Private Sub Form_Resize()
    ...
    With fraButtons
        .Top = tdgBank.Top + tdgBank.Height
        .Left = 50
        .Width = Me.ScaleWidth - 75
    End With
