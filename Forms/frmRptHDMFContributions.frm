...
Begin VB.Form frmRptHDMFContributions 
   ClientHeight    =   8085
   ClientLeft      =   60
   ClientTop       =   60
   ClientWidth     =   10215
   ...
   Begin VB.Frame fraParmtr 
      ...
      Begin OsenXPCntrl.OsenXPButton cmdView 
         ...
         MCOL            =   12632256
         MPTR            =   0
         MICON           =   "frmRptHDMFContributions.frx":0000
         PICN            =   "frmRptHDMFContributions.frx":001C
         UMCOL           =   -1  'True
         SOFT            =   0   'False
         PICPOS          =   0
         NGREY           =   0   'False
      ...
      Begin TrueOleDBList80.TDBCombo tdbCostCenter 
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRptHDMFContributions.frx":05B6
      ...
      Begin TrueOleDBList80.TDBCombo tdbDivision 
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRptHDMFContributions.frx":0660
         _StyleDefs(0)   =   "_StyleRoot:id=0,.parent=-1,.alignment=3,.valignment=0,.bgcolor=&H80000005&"
         _StyleDefs(1)   =   ":id=0,.fgcolor=&H80000008&,.wraptext=0,.locked=0,.transparentBmp=0"
         _StyleDefs(2)   =   ":id=0,.fgpicPosition=0,.bgpicMode=0,.appearance=0,.borderSize=0,.ellipsis=0"
         _StyleDefs(3)   =   ":id=0,.borderColor=&H80000005&,.borderType=0,.bold=0,.fontsize=825,.italic=0"
      ...
      Begin TrueOleDBList80.TDBCombo tdbMonth 
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRptHDMFContributions.frx":070A
         _StyleDefs(0)   =   "_StyleRoot:id=0,.parent=-1,.alignment=3,.valignment=0,.bgcolor=&H80000005&"
         _StyleDefs(1)   =   ":id=0,.fgcolor=&H80000008&,.wraptext=0,.locked=0,.transparentBmp=0"
         _StyleDefs(2)   =   ":id=0,.fgpicPosition=0,.bgpicMode=0,.appearance=0,.borderSize=0,.ellipsis=0"
         _StyleDefs(3)   =   ":id=0,.borderColor=&H80000005&,.borderType=0,.bold=0,.fontsize=825,.italic=0"
      ...
      Begin TDBNumber6Ctl.TDBNumber txtPayYear 
         ...
         _ExtentX        =   2646
         _ExtentY        =   556
         Calculator      =   "frmRptHDMFContributions.frx":07B4
         Caption         =   "frmRptHDMFContributions.frx":07D4
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Verdana"
            Size            =   8.25
            Charset         =   0
            ...
            Strikethrough   =   0   'False
         EndProperty
         DropDown        =   "frmRptHDMFContributions.frx":083A
         Keys            =   "frmRptHDMFContributions.frx":0858
         Spin            =   "frmRptHDMFContributions.frx":08A2
         AlignHorizontal =   1
         AlignVertical   =   0
         Appearance      =   0
   ...
   End
End
Attribute VB_Name = "frmRptHDMFContributions"
...
Private Sub cmdView_Click()
    ...
    NetOpen rsPayrollPeriod, "SELECT percode FROM payrollperiod WHERE paymonth='" & tdbMonth.Text & "' AND payyear ='" & txtPayYear.Text & "'"
    With rsPayrollPeriod
      If .RecordCount > 0 Then
        .MoveFirst
        Do While Not .EOF
          If Trim(mPercodes) = "" Then
            mPercodes = !percode
          Else
            mPercodes = mPercodes & "," & !percode
          End If
          .MoveNext
        Loop
      Else
    ...
    Set CrxRep = CrxApp.OpenReport(App.Path & "\reports\HDMFContributions.rpt")
    ...
    If Trim(tdbDivision.Text) <> "" And Not IsNull(tdbDivision.SelectedItem) And tdbDivision.ApproxCount <> 0 And tdbDivision.Enabled = True Then
        If Trim(tdbCostCenter.Text) <> "" And Not IsNull(tdbCostCenter.SelectedItem) And tdbCostCenter.ApproxCount <> 0 And tdbCostCenter.Enabled = True Then
                mAddQuery = " and divisioncode = " & tdbDivision.BoundText & " and costcentercode = '" & tdbCostCenter.BoundText & "' "
        Else
            mAddQuery = " and divisioncode = " & tdbDivision.BoundText & " "
        End If
    Else
        mAddQuery = ""
    End If
    
    CrxRep.ParameterFields.GetItemByName("mMonth").AddCurrentValue CStr(tdbMonth.Text)
    CrxRep.ParameterFields.GetItemByName("mYear").AddCurrentValue CStr(txtPayYear.Text)
    CrxRep.ParameterFields.GetItemByName("mPercodes").AddCurrentValue mPercodes
