...
Begin VB.Form frmRptPayslips 
   ...
   Begin VB.Frame fraParmtr 
      ...
      Begin VB.Frame Frame2 
         ...
         Top             =   1365
         Width           =   3225
         Begin VB.OptionButton optRankNFile 
            Appearance      =   0  'Flat
            BackColor       =   &H00F6F8F8&
            Caption         =   "Rank && File"
            BeginProperty Font 
            ...
            Width           =   2595
         End
         Begin VB.OptionButton optConfidential 
            Appearance      =   0  'Flat
            BackColor       =   &H00F6F8F8&
      ...
      Begin TrueOleDBList80.TDBCombo tdbPayrollPeriod 
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRptPayslips.frx":0000
      ...
      Begin TrueOleDBList80.TDBCombo tdbEmployee 
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRptPayslips.frx":00AA
         _StyleDefs(0)   =   "_StyleRoot:id=0,.parent=-1,.alignment=3,.valignment=0,.bgcolor=&H80000005&"
         _StyleDefs(1)   =   ":id=0,.fgcolor=&H80000008&,.wraptext=0,.locked=0,.transparentBmp=0"
         _StyleDefs(2)   =   ":id=0,.fgpicPosition=0,.bgpicMode=0,.appearance=0,.borderSize=0,.ellipsis=0"
         _StyleDefs(3)   =   ":id=0,.borderColor=&H80000005&,.borderType=0,.bold=0,.fontsize=825,.italic=0"
      ...
      Begin OsenXPCntrl.OsenXPButton cmdView 
         ...
         MCOL            =   12632256
         MPTR            =   0
         MICON           =   "frmRptPayslips.frx":0154
         PICN            =   "frmRptPayslips.frx":0170
         UMCOL           =   -1  'True
         SOFT            =   0   'False
         PICPOS          =   0
         NGREY           =   0   'False
      ...
      Begin TrueOleDBList80.TDBCombo tdbCostCenter 
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRptPayslips.frx":070A
         _StyleDefs(0)   =   "_StyleRoot:id=0,.parent=-1,.alignment=3,.valignment=0,.bgcolor=&H80000005&"
         _StyleDefs(1)   =   ":id=0,.fgcolor=&H80000008&,.wraptext=0,.locked=0,.transparentBmp=0"
         _StyleDefs(2)   =   ":id=0,.fgpicPosition=0,.bgpicMode=0,.appearance=0,.borderSize=0,.ellipsis=0"
         _StyleDefs(3)   =   ":id=0,.borderColor=&H80000005&,.borderType=0,.bold=0,.fontsize=825,.italic=0"
      ...
      Begin TrueOleDBList80.TDBCombo tdbDivision 
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRptPayslips.frx":07B4
         _StyleDefs(0)   =   "_StyleRoot:id=0,.parent=-1,.alignment=3,.valignment=0,.bgcolor=&H80000005&"
         _StyleDefs(1)   =   ":id=0,.fgcolor=&H80000008&,.wraptext=0,.locked=0,.transparentBmp=0"
         _StyleDefs(2)   =   ":id=0,.fgpicPosition=0,.bgpicMode=0,.appearance=0,.borderSize=0,.ellipsis=0"
         _StyleDefs(3)   =   ":id=0,.borderColor=&H80000005&,.borderType=0,.bold=0,.fontsize=825,.italic=0"
   ...
   End
End
Attribute VB_Name = "frmRptPayslips"
...
Private Sub cmdView_Click()
    ...
    If Trim(tdbPayrollPeriod.Text) = "" Or IsNull(tdbPayrollPeriod.SelectedItem) Or tdbPayrollPeriod.ApproxCount <= 0 Then
        MsgBox "pleaese select a payroll period.", vbExclamation + vbOKOnly
    ...
    'CrxApp.LogOnServer "{MySQL ODBC 3.51 Driver}", "SQLServerName", "SQLDatabase", "SQLUsername", "SQLPassword"
    If CDbl(tdbPayrollPeriod.Columns("percode").Text) <= 164 Then
      If optEmployee.Value = True Then
        Set CrxRep = CrxApp.OpenReport(App.Path & "\reports\Payslips20160125_dontdelete.rpt")
      Else
          Set CrxRep = CrxApp.OpenReport(App.Path & "\reports\PayslipsDivCostcenter20160211dontdelete.rpt")
      End If
    Else
      If optEmployee.Value = True Then
        Set CrxRep = CrxApp.OpenReport(App.Path & "\reports\payslips.rpt")
      Else
          Set CrxRep = CrxApp.OpenReport(App.Path & "\reports\PayslipsDivCostcenter.rpt")
      End If
    End If
    
    'CrxRep.Database.LogOnServer "{MySQL ODBC 3.51 Driver}", "SQLServerName", "SQLDatabase", "SQLUsername", "SQLPassword"
...
'        crxDatabaseTable.ConnectionProperties.Add "Provider", "{MySQL ODBC 3.51 Driver}"
'        crxDatabaseTable.ConnectionProperties.Add "Server", SQLServerName
'        crxDatabaseTable.ConnectionProperties.Add "Username", SQLUsername
'        crxDatabaseTable.ConnectionProperties.Add "Password", SQLPassword
'        crxDatabaseTable.ConnectionProperties.Add "Database", SQLDatabase
'        crxDatabaseTable.ConnectionProperties.Add "Port", "3306"
        ...
        'crxDatabaseTable.ConnectionProperties.Item("connection string") = "DRIVER={MySQL ODBC 3.51 Driver}; SERVER=" & SQLServerName & "; DATABASE=" & SQLDatabase & "; UID=" & SQLUsername & "; PWD=" & SQLPassword & "; PORT='" & SQLPort & "'"
    ...
    If optConfidential.Value = True Then
      mAddQuery = mAddQuery & " and x1.confidential='Y' "
    ElseIf optRankNFile.Value = True Then
      mAddQuery = mAddQuery & " and x1.confidential='N' "
    End If
    
    'CrxRep.ParameterFields.GetItemByName("mreporttitle").AddCurrentValue TitleBar.Caption
    CrxRep.ParameterFields.GetItemByName("mPercode").AddCurrentValue CInt(tdbPayrollPeriod.Columns("percode").Text)
    'CrxRep.ParameterFields.GetItemByName("mperdesc").AddCurrentValue tdbPayrollPeriod.Columns("description").Text
    CrxRep.ParameterFields.GetItemByName("mAddQuery").AddCurrentValue mAddQuery
    CrxRep.ParameterFields.GetItemByName("mDatecovered").AddCurrentValue tdbPayrollPeriod.Columns("wrkdatefrom").Text & " - " & tdbPayrollPeriod.Columns("wrkdateto").Text
    
    xrpt.ReportSource = CrxRep
    xrpt.ViewReport
    xrpt.Zoom 130
    If optEmployee.Value = True Then
        xrpt.EnableGroupTree = False
    Else
        xrpt.EnableGroupTree = True
    End If
...
Private Sub Form_Load()
    ...
    'bind_tdb ConMain, tdbPayrollPeriod, "select percode, description,wrkdatefrom,wrkdateto,payfreqcode from payrollperiod desc", "description", "percode"
    bind_tdb ConMain, tdbPayrollPeriod, "select percode, description,wrkdatefrom,wrkdateto,payfreqcode from payrollperiod order by percode desc", "description", "percode", True
...
Private Sub tdbcostcenter_ItemChange()
    ...
    bind_tdb ConMain, tdbEmployee, "select employeecode,concat(lastname, ', ', firstname,' ',middlename) fullname from employee " & _
        "where divisioncode = " & tdbDivision.BoundText & " and costcentercode = '" & tdbCostCenter.BoundText & "' and payfreqcode = '" & tdbPayrollPeriod.Columns("payfreqcode").Text & "' order by concat(lastname, ', ', firstname,' ',middlename)", "fullname", "employeecode"
