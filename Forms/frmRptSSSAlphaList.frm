...
Begin VB.Form frmRptSSSAlphaList 
   ClientHeight    =   8295
   ClientLeft      =   60
   ClientTop       =   60
   ClientWidth     =   9330
   ...
   Begin VB.Frame fraParmtr 
      ...
      Begin OsenXPCntrl.OsenXPButton cmdView 
         ...
         MCOL            =   12632256
         MPTR            =   0
         MICON           =   "frmRptSSSAlphaList.frx":0000
         PICN            =   "frmRptSSSAlphaList.frx":001C
         UMCOL           =   -1  'True
         SOFT            =   0   'False
         PICPOS          =   0
         NGREY           =   0   'False
      ...
      Begin TrueOleDBList80.TDBCombo tdbDivision 
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRptSSSAlphaList.frx":05B6
      ...
      Begin TrueOleDBList80.TDBCombo tdbCostCenter 
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRptSSSAlphaList.frx":0660
         _StyleDefs(0)   =   "_StyleRoot:id=0,.parent=-1,.alignment=3,.valignment=0,.bgcolor=&H80000005&"
         _StyleDefs(1)   =   ":id=0,.fgcolor=&H80000008&,.wraptext=0,.locked=0,.transparentBmp=0"
         _StyleDefs(2)   =   ":id=0,.fgpicPosition=0,.bgpicMode=0,.appearance=0,.borderSize=0,.ellipsis=0"
         _StyleDefs(3)   =   ":id=0,.borderColor=&H80000005&,.borderType=0,.bold=0,.fontsize=825,.italic=0"
   ...
   Begin LinkProPayroll.b8ChildTitleBar TitleBar 
      ...
      _ExtentX        =   4604
      _ExtentY        =   609
      Caption         =   "Employee Pre-validation File"
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Tahoma"
         Size            =   8.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmRptSSSAlphaList"
...
Private Sub cmdView_Click()
    ...
    If optEmployee.Value = True Then
        Set CrxRep = CrxApp.OpenReport(App.Path & "\reports\SSSEPF_Employees.rpt")
   
    ElseIf optDivision.Value = True Then
        Set CrxRep = CrxApp.OpenReport(App.Path & "\reports\SSSEPF_Division.rpt")
...
'        End If
    ElseIf optCostcenter.Value = True Then
        Set CrxRep = CrxApp.OpenReport(App.Path & "\reports\SSSEPF_CostCenter.rpt")
'        If Trim(tdbDivision.Text) <> "" And Not IsNull(tdbDivision.SelectedItem) And tdbDivision.ApproxCount > 0 Then
...
'        End If
    ...
    If Trim(tdbDivision.Text) <> "" And Not IsNull(tdbDivision.SelectedItem) And tdbDivision.ApproxCount > 0 Then
      mAddQuery = " where b.divisioncode = " & tdbDivision.BoundText & " "
      If Trim(tdbCostCenter.Text) <> "" And Not IsNull(tdbCostCenter.SelectedItem) And tdbCostCenter.ApproxCount > 0 Then
        mAddQuery = mAddQuery & " and b.costcentercode = " & tdbCostCenter.BoundText & " "
      End If
    End If
