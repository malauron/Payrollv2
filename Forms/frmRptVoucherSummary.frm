...
Begin VB.Form frmRptVoucherSummary 
   BackColor       =   &H00FFFFFF&
   ClientHeight    =   7530
   ClientLeft      =   60
   ClientTop       =   60
   ...
   Begin VB.Frame fraParmtr 
      ...
      Begin TrueOleDBList80.TDBCombo tdbEmployee 
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRptVoucherSummary.frx":0000
      ...
      Begin OsenXPCntrl.OsenXPButton cmdView 
         ...
         MCOL            =   12632256
         MPTR            =   0
         MICON           =   "frmRptVoucherSummary.frx":00AA
         PICN            =   "frmRptVoucherSummary.frx":00C6
         UMCOL           =   -1  'True
         SOFT            =   0   'False
         PICPOS          =   0
         NGREY           =   0   'False
         ...
         VALUE           =   0   'False
      End
      Begin TrueOleDBList80.TDBCombo tdbVoucherType 
         Height          =   345
         Left            =   150
         TabIndex        =   2
         Tag             =   "Municipal"
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRptVoucherSummary.frx":0660
         _StyleDefs(0)   =   "_StyleRoot:id=0,.parent=-1,.alignment=3,.valignment=0,.bgcolor=&H80000005&"
         _StyleDefs(1)   =   ":id=0,.fgcolor=&H80000008&,.wraptext=0,.locked=0,.transparentBmp=0"
         _StyleDefs(2)   =   ":id=0,.fgpicPosition=0,.bgpicMode=0,.appearance=0,.borderSize=0,.ellipsis=0"
         _StyleDefs(3)   =   ":id=0,.borderColor=&H80000005&,.borderType=0,.bold=0,.fontsize=825,.italic=0"
      ...
      Begin TrueOleDBList80.TDBCombo tdbValidityPeriod 
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRptVoucherSummary.frx":070A
         _StyleDefs(0)   =   "_StyleRoot:id=0,.parent=-1,.alignment=3,.valignment=0,.bgcolor=&H80000005&"
         _StyleDefs(1)   =   ":id=0,.fgcolor=&H80000008&,.wraptext=0,.locked=0,.transparentBmp=0"
         _StyleDefs(2)   =   ":id=0,.fgpicPosition=0,.bgpicMode=0,.appearance=0,.borderSize=0,.ellipsis=0"
         _StyleDefs(3)   =   ":id=0,.borderColor=&H80000005&,.borderType=0,.bold=0,.fontsize=825,.italic=0"
      ...
      Begin TrueOleDBList80.TDBCombo tdbSort 
         ...
         MaxComboItems   =   10
         AddItemSeparator=   ";"
         _PropDict       =   $"frmRptVoucherSummary.frx":07B4
         _StyleDefs(0)   =   "_StyleRoot:id=0,.parent=-1,.alignment=3,.valignment=0,.bgcolor=&H80000005&"
         _StyleDefs(1)   =   ":id=0,.fgcolor=&H80000008&,.wraptext=0,.locked=0,.transparentBmp=0"
         _StyleDefs(2)   =   ":id=0,.fgpicPosition=0,.bgpicMode=0,.appearance=0,.borderSize=0,.ellipsis=0"
         _StyleDefs(3)   =   ":id=0,.borderColor=&H80000005&,.borderType=0,.bold=0,.fontsize=825,.italic=0"
   ...
   End
End
Attribute VB_Name = "frmRptVoucherSummary"
...
Private Sub cmdView_Click()
    ...
    If Trim(tdbSort.Text) = "" Or IsNull(tdbSort.SelectedItem) Or tdbSort.ApproxCount <= 0 Then
      MsgBox "Please choose a sort option.", vbExclamation + vbOKOnly
      tdbSort.SetFocus
      Exit Sub
    End If
    
    If tdbSort.BoundText = "vouchercode" Then
      mSort = " order by x1.vouchercode "
    ElseIf tdbSort.BoundText = "fullname" Then
      mSort = "order by x2.lastname,x2.firstname,x2.middlename "
    ElseIf tdbSort.BoundText = "amount" Then
      mSort = " order by x1.amount "
    End If
...
'    If Trim(tdbVoucherType.Text) = "" Or IsNull(tdbVoucherType.SelectedItem) Or tdbVoucherType.ApproxCount <= 0 Then
'        MsgBox "Please select a voucher type.", vbExclamation + vbOKOnly
'        tdbVoucherType.SetFocus
'        Exit Sub
'    End If
    ...
    Set CrxRep = CrxApp.OpenReport(App.Path & "\reports\rptVoucherSummary.rpt")
    ...
    CrxRep.ParameterFields.GetItemByName("mSort").AddCurrentValue mSort
...
Private Sub Form_Load()
    ...
    With rsTmp
        ...
        .AddNew
        Select Case i
            Case 1: .Fields("code") = "vouchercode"
                    .Fields("description") = "Voucher Code"
            Case 2: .Fields("code") = "fullname"
                    .Fields("description") = "Employee"
            Case 3: .Fields("code") = "amount"
...
Private Sub tdbSort_GotFocus()
  tdbSort.SelStart = 0
  tdbSort.SelLength = Len(tdbSort.Text)
End Sub
...
Private Sub tdbVoucherType_GotFocus()

    If Trim(tdbVoucherType.Text) <> "" And Not IsNull(tdbVoucherType.SelectedItem) And tdbVoucherType.ApproxCount > 0 Then
        tdbVoucherType.Tag = tdbVoucherType.BoundText
    Else
        tdbVoucherType.Tag = ""
    End If
    
'    bind_tdb ConMain, tdbVoucherType, "select Voucherscode,Vouchersname from Vouchers where isactive = 'Y' order by Vouchersname", "Vouchersname", "Voucherscode"
    
    If mVoucherType <> "" Then
        tdbVoucherType.BoundText = mVoucherType
    Else
        tdbVoucherType.BoundText = ""
    End If
    
End Sub

Private Sub tdbVoucherType_KeyPress(KeyAscii As Integer)
    ...
    Else
        ...
        SearchList KeyAscii, tdbVoucherType, tdbVoucherType.RowSource, tdbVoucherType.Text
...
Public Sub Get_List()
    ...
    If Trim(tdbVoucherType.Text) <> "" And Not IsNull(tdbVoucherType.SelectedItem) And tdbVoucherType.ApproxCount > 0 Then
        mVoucherType = tdbVoucherType.BoundText
    Else
        mVoucherType = ""
    End If
    
    'bind_tdb ConMain, tdbEmployee, "select x1.employeecode, concat(x2.lastname,', ',x2.firstname,' ',x2.middlename) employeename from vouchers x1 " & _
               "left outer join employee x2 on x1.employeecode = x2.employeecode " & _
               "left outer join vouchervalidperiod x3 on x1.vouchervalidperiodcode = x3.vouchervalidperiodcode  where x1.vouchervalidperiodcode = " & mVoucherValidPeriodCode & " and x1.vouchertype = '" & mVoucherType & "' order by x2.lastname,x2.firstname,x2.middlename ", "employeename", "employeecode"
